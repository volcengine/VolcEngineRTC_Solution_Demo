...
type videoInteractReq struct {
	RoomID     string `json:"room_id"`
	UserID     string `json:"user_id"`
	LoginToken string `json:"login_token"`
}

func closeVideoInteract(ctx context.Context, param *vc_control.TEventParam) {
	logs.Infof("closeVideoInteract:%+v", param)
	var p videoInteractReq
	...
	err = room.CloseInteract(ctx)
	if err != nil {
		logs.Errorf("close inter failed,error:%s", err)
	...
	room.InformRoom(ctx, edu_models.OnCloseVideoInteract, &edu_models.NoticeRoom{RoomID: room.GetRoomID()})
