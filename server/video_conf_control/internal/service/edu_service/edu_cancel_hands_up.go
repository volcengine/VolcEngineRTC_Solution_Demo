...
type cancelHandsUpReq struct {
	UserID     string `json:"user_id"`
	RoomID     string `json:"room_id"`
	LoginToken string `json:"login_token"`
}

func cancelHandsUp(ctx context.Context, param *vc_control.TEventParam) {
	logs.Infof("cancelHandsUp:%+v", param)
	var p cancelHandsUpReq
	...
	service_utils.Push2Client(ctx, param, err, &edu_models.NoticeRoom{RoomID: room.GetRoomID(), UserID: user.GetUserID()})
}
