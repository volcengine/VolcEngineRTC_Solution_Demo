...
func turnOnMic(ctx context.Context, param *vc_control.TEventParam) {
	...
	err = user.Save(ctx)
	if err != nil {
		logs.Errorf("turn on mic failed,error:%s", err)
		service_utils.Push2ClientWithoutReturn(ctx, param, custom_error.InternalError(err))
		return
	}

	room.InformRoom(ctx, edu_models.OnTeacherMicOn, edu_models.NoticeRoom{RoomID: room.GetRoomID()})
