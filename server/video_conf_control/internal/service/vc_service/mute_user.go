...
type muteUserParam struct {
	LoginToken string `json:"login_token"`
	UserID     string `json:"user_id"`
}

func muteUser(ctx context.Context, param *vc_control.TEventParam) {
	var p muteUserParam
	...
	service_utils.Push2ClientWithoutReturn(ctx, param, user.MuteUser(ctx, p.UserID))
}
