...
type raiseHandsMicParam struct {
	LoginToken string `json:"login_token"`
}

func raiseHandsMic(ctx context.Context, param *vc_control.TEventParam) {
	var p raiseHandsMicParam
	...
	err = user.RaiseHandsMic(ctx)
	if err != nil {
		logs.Errorf("failed to raise hands mic, err %v", err)
		service_utils.Push2ClientWithoutReturn(ctx, param, err)
		return
	}
	...
	user.GetRoom().Inform(ctx, cs_models.OnCsRaiseHandsMic, user.GenerateInformUserInfo())
}
