...
type leaveMeetingParam struct {
	LoginToken string `json:"login_token"`
}

func leaveMeeting(ctx context.Context, param *vc_control.TEventParam) {
	var p leaveMeetingParam
	...
	err = user.LeaveMeeting(ctx)
	if err != nil {
		logs.Errorf("failed to leave meetings, err: %v", err)
		service_utils.Push2ClientWithoutReturn(ctx, param, err)
		return
	}
